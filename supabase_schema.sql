-- ============================================================================
-- VERITAS & LEX - SUPABASE DATABASE SCHEMA
-- Multi-Tenant White-Label Legal Architecture
-- ============================================================================
...
-- Submissão pública de audiências e newsletter (clientes)
...
-- Acesso administrativo irrestrito para usuários autenticados
CREATE POLICY "Auth all on firm_configs" ON public.firm_configs FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Auth all on intake_protocols" ON public.intake_protocols FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Auth all on practice_areas" ON public.practice_areas FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Auth all on law_review_articles" ON public.law_review_articles FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Auth all on newsletter_subscribers" ON public.newsletter_subscribers FOR ALL USING (auth.role() = 'authenticated');
