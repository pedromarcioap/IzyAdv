...
export const SessionSecurityModal: React.FC<SessionSecurityModalProps> = ({ isOpen, onClose, embedded = false }) => {
    ...
    const hasCurrentSession = listing.sessions.some((session) => session.isCurrent);

    const cardContent = (
        <div
            className={`w-full max-w-xl rounded-2xl border border-[#D4AF37]/50 bg-[#13060A] p-6 shadow-2xl ${
                embedded ? 'mx-auto' : ''
            }`}
        >
            ...
            {message && (
                <output className="mb-3 block rounded border border-emerald-800 bg-emerald-950/40 p-3 font-data-mono text-xs text-emerald-200">
                    {message}
                </output>
            )}
            ...
            <div className="flex flex-col gap-2 border-t border-[#431520] pt-4">
                ...
                <p className="font-data-mono text-[10px] leading-relaxed text-[#7D6B60]">
                    {hasCurrentSession ? 'A sessão marcada como "esta sessão" é a que você está usando agora. ' : ''}
    ...
    return (
        <dialog
            open
            className="fixed inset-0 z-[70] m-0 border-none p-4 w-full h-full max-w-none max-h-none bg-black/85 flex items-center justify-center overflow-y-auto backdrop-blur-md animate-fade-in text-inherit"
